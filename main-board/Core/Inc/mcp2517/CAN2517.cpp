...
// by Pierre Molinaro
// https://github.com/pierremolinaro/CAN2517
//
//------------------------------------------------------------------------------
...
//     Nick Kirkby for having signaled me this point, see https://github.com/pierremolinaro/CAN2517/issues/5);
...
//------------------------------------------------------------------------------
  ...
  static void myESP32Task (void * pData) {
    CAN2517 * canDriver = (CAN2517 *) pData ;
    while (1) {
...
CAN2517::CAN2517 (SPI_HandleTypeDef* hspi, // Hardware SPI object
		  GPIO_TypeDef* csPort,
		  uint16_t csPin,
		  GPIO_TypeDef* intPort,
		  uint16_t intPin
) : // INT output of MCP2517FD
//mSPISettings (),
//mSPI (inSPI),
//mCS (inCS),
//mINT (inINT),
...
uint32_t CAN2517::begin (const MCP2517Settings & inSettings,
...
//----------------------------------- Check Filter definition
  ...
  if (inFilters.filterStatus () != CANFilters::kFiltersOk) {
...
//     Bits 5-6: Clovk Output Divisor
  if (errorCode == 0) {
    ...
    case MCP2517Settings::OSC_4MHz:
    case MCP2517Settings::OSC_20MHz:
    case MCP2517Settings::OSC_40MHz:
      break ;
    case MCP2517Settings::OSC_4MHz_DIVIDED_BY_2:
    case MCP2517Settings::OSC_20MHz_DIVIDED_BY_2:
    case MCP2517Settings::OSC_40MHz_DIVIDED_BY_2:
      osc =  1 << 4 ; // Divide by 2
      break ;
    case MCP2517Settings::OSC_4MHz10xPLL_DIVIDED_BY_2 :
      pll = 1 ; // Enable 10x PLL
      osc =  1 << 4 ; // Divide by 2
      break ;
    case MCP2517Settings::OSC_4MHz10xPLL :
      pll = 1 ; // Enable 10x PLL
      break ;
    }
    osc |= pll ;
    if (inSettings.mCLKOPin != MCP2517Settings::SOF) {
...
//----------------------------------- Install interrupt, configure external interrupt
  ...
  //----------------------------------- Configure receive filters
    ...
    CANFilters::Filter * filter = inFilters.mFirstFilter ;
  ...
  //----------------------------------- Wait (2 ms max) until requested mode is reached
    ...
    #ifdef ARDUINO_ARCH_ESP32
      xTaskCreate (myESP32Task, "CAN2517Handler", 1024, this, 16, NULL) ;
    #endif
    if (m_intPort != nullptr) { // 255 means interrupt is not used
      #ifdef ARDUINO_ARCH_ESP32
...
//        attachInterrupt (itPin, inInterruptServiceRoutine, LOW) ;
          // TODO: add interrupt pin handling
      #endif
    }
  }
//---
...
bool CAN2517::tryToSend (const CANMessage & inMessage) {
//  mSPI.beginTransaction (mSPISettings) ;
    #ifdef ARDUINO_ARCH_ESP32
      taskDISABLE_INTERRUPTS () ;
    #else
...
bool CAN2517::enterInTransmitBuffer (const CANMessage & inMessage) {
  bool result ;
...
void CAN2517::appendInControllerTxFIFO (const CANMessage & inMessage) {
...
//    mSPI.transfer (buff, 18) ;
    HAL_SPI_Transmit(m_hspi, buff, sizeof (buff), HAL_MAX_DELAY);
  deassertCS () ;
...
bool CAN2517::sendViaTXQ (const CANMessage & inMessage) {
  bool ok = mUsesTXQ ;
  if (ok) {
...
bool CAN2517::receive (CANMessage & outMessage) {
  #ifdef ARDUINO_ARCH_ESP32
    mSPI.beginTransaction (mSPISettings) ; // For ensuring mutual exclusion access
    taskDISABLE_INTERRUPTS () ;
  #else
...
bool CAN2517::dispatchReceivedMessage (const tFilterMatchCallBack inFilterMatchCallBack) {
...
void CAN2517::transmitInterrupt (void) {
  CANMessage message ;
...
void CAN2517::receiveInterrupt (void) {
...
void CAN2517::assertCS (void) {
	HAL_GPIO_WritePin(m_csPort, m_csPin, GPIO_PIN_RESET);
}

//------------------------------------------------------------------------------

void CAN2517::deassertCS (void) {
	HAL_GPIO_WritePin(m_csPort, m_csPin, GPIO_PIN_SET);
}

//------------------------------------------------------------------------------

void CAN2517::writeRegister32Assume_SPI_transaction (const uint16_t inRegisterAddress, const uint32_t inValue) {
...
uint32_t CAN2517::readRegister32Assume_SPI_transaction (const uint16_t inRegisterAddress) {
...
void CAN2517::writeRegister8Assume_SPI_transaction (const uint16_t inRegisterAddress, const uint8_t inValue) {
...
uint8_t CAN2517::readRegister8Assume_SPI_transaction (const uint16_t inRegisterAddress) {
...
void CAN2517::writeRegister8 (const uint16_t inRegisterAddress, const uint8_t inValue) {
//  mSPI.beginTransaction (mSPISettings) ;
    #ifdef ARDUINO_ARCH_ESP32
      taskDISABLE_INTERRUPTS () ;
    #else
...
uint8_t CAN2517::readRegister8 (const uint16_t inRegisterAddress) {
//  mSPI.beginTransaction (mSPISettings) ;
    #ifdef ARDUINO_ARCH_ESP32
      taskDISABLE_INTERRUPTS () ;
    #else
...
void CAN2517::writeRegister32 (const uint16_t inRegisterAddress, const uint32_t inValue) {
//  mSPI.beginTransaction (mSPISettings) ;
    #ifdef ARDUINO_ARCH_ESP32
      taskDISABLE_INTERRUPTS () ;
    #else
...
uint32_t CAN2517::readRegister32 (const uint16_t inRegisterAddress) {
//  mSPI.beginTransaction (mSPISettings) ;
    #ifdef ARDUINO_ARCH_ESP32
      taskDISABLE_INTERRUPTS () ;
    #else
...
uint32_t CAN2517::errorCounters (void) {
...
CAN2517::OperationMode CAN2517::currentOperationMode (void) {
  const uint8_t mode = readRegister8 (C1CON_REGISTER + 2) >> 5 ;
  return CAN2517::OperationMode (mode) ;
}

//------------------------------------------------------------------------------

bool CAN2517::recoverFromRestrictedOperationMode (void) {
   bool recoveryDone = false ;
   if (currentOperationMode () == CAN2517::RestrictedOperation) { // In Restricted Operation Mode
  //----------------------------------- Request mode (C1CON_REGISTER + 3)
  //  bits 7-4: Transmit Bandwith Sharing Bits ---> 0
  //  bit 3: Abort All Pending Transmissions bit --> 0
  ...
  //----------------------------------- Wait (10 ms max) until requested mode is reached
    bool wait = true ;
    const uint32_t deadline = HAL_GetTick() + 10 ;
    while (wait) {
      const uint8_t actualMode = (readRegister8 (C1CON_REGISTER + 2) >> 5) & 0x07 ;
...
uint32_t CAN2517::diagInfos (const int inIndex) { // thanks to Flole998 and turmary
...
void CAN2517::gpioSetMode (const uint8_t inPin, const uint8_t inMode) {
  if (inPin <= 1) {
...
void CAN2517::gpioWrite (const uint8_t inPin, const uint8_t inLevel) {
  if (inPin <= 1) {
...
bool CAN2517::gpioRead (const uint8_t inPin) {
...
void CAN2517::configureGPIO0AsXSTBY (void) {
  uint8_t value = readRegister8 (IOCON_REGISTER_00_07) ;
