...
void ADS124_Init(ADS124_HandleTypeDef *h, SPI_HandleTypeDef *hspi)
{
	h->hspi = hspi;
...
HAL_StatusTypeDef ADS124_WriteRegister(ADS124_HandleTypeDef *h, uint8_t start_addr, uint8_t *data, uint8_t n_regs)
{
...
HAL_StatusTypeDef ADS124_ReadRegister(ADS124_HandleTypeDef *h, uint8_t start_addr, uint8_t *data, uint8_t n_regs)
{
...
void ADS124_SetDataRate(ADS124_HandleTypeDef *h, uint8_t dr_code) {
    ADS124_WriteRegister(h, ADS124_REG_DATARATE, &dr_code, 1);
}

void ADS124_SelectChannel(ADS124_HandleTypeDef *h, uint8_t ch) {
    uint8_t mux = (ch << 4) | 0x08;
    // PSEL = ch (0…5 → AIN0…AIN5), NSEL = 8 → AINCOM
    ADS124_WriteRegister(h, ADS124_REG_INPMUX, &mux, 1);
}
